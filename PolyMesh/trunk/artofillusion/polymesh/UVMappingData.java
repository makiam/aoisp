...
/**
 * This class holds unfolded meshes and UV mappiung editor information
...
public class UVMappingData {
	...
	public UVMeshMapping duplicate() {
	    ...
	    newMapping.edgeColor = new Color(edgeColor.getRed(),
	...
	public UVMeshMapping(DataInputStream in) throws IOException,
	...
	throws IOException {
	    ...
	    for (int i = 0; i < mappings.size(); i++) {
		mappings.get(i).writeToFile(out);
	    }
	    out.writeInt(sampling);
	}

	public UVMappingData(DataInputStream in) throws IOException,
	InvalidObjectException {
	    short version = in.readShort();
	    ...
	    mappings = new ArrayList<UVMeshMapping>();
	    for (int i = 0; i < count; i++) {
		mappings.add(new UVMeshMapping(in));
	    }
	    sampling = in.readInt();
	}
