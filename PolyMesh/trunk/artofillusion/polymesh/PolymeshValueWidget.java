...
public class PolymeshValueWidget extends BorderContainer
{
    ...
    private Runnable runCallback, validateCallback, abortCallback;
    private boolean temporaryRange;

    public PolymeshValueWidget()
    {
        ...
        try
        {
            ...
            maxLabel.setText( PMTranslate.text( maxLabel.getText() ) );
            maxSpinner = ( (BSpinner) decoder.getObject( "maxSpinner" ) );
            validateButton = ( (BButton) decoder.getObject( "validateButton" ) );
            validateButton.setText( PMTranslate.text( validateButton.getText() ) );
            abortButton = ( (BButton) decoder.getObject( "abortButton" ) );
            abortButton.setText( PMTranslate.text( abortButton.getText() ) );
            BLabel minLabel = ( (BLabel) decoder.getObject( "minLabel" ) );
            minLabel.setText( PMTranslate.text( minLabel.getText() ) );
            retainValueCB = ( (BCheckBox) decoder.getObject( "retainValueCB" ) );
            retainValueCB.setText( PMTranslate.text( retainValueCB.getText() ) );
     ...
     */
   public void activate(Runnable runCallback, Runnable validateCallback, Runnable abortCallback)
    {
        activate( 0.0, runCallback, validateCallback, abortCallback );
    }
     ...
     * @param abortCallback  Callback to call when process is cancelled by user
     */
   public void activate( double val, Runnable runCallback, Runnable validateCallback, Runnable abortCallback)
   {
