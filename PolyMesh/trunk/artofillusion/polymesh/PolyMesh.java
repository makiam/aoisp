...
public class PolyMesh extends Object3D implements Mesh, FacetedMesh {
	...
	public RenderingMesh getRenderingMesh(double tol, boolean interactive,
		...
		if (smoothingMethod == Mesh.APPROXIMATING && !finalSmoothing) {
			...
			/*
			 * System.out.println( (mesh.time1 - mesh.time2) + " t2: " +
			 * mesh.time2 + " t3: " + mesh.time3 + " t4: " + (mesh.time4 -
			 * mesh.time2 - mesh.time3) + " t5: " + (mesh.time5 - mesh.time4) + "
			 * t6: " + (mesh.time6 - mesh.time5) + " t7: " + (mesh.time7 -
			 * mesh.time6)); System.out.println(System.currentTimeMillis() - t);
			 */
			// System.out.println(System.currentTimeMillis() - t);
		}
		// to comment out
		...
		/*
		 * if ( tmpSmoothingMethod != Mesh.NO_SMOOTHING ) { vs = new Vector(); }
		 */
		closed = true;
		for (int i = 0; i < edges.length / 2; ++i) {
		...
		for (int i = 0; i < faces.length; ++i) {
			...
			if (vf.length == 3) {
				...
				// if ( s1 != null )
				// addSmoothness( getFaceEdges( faces[i] ), s1, s2, s3, vf[0],
				// vf[1], vf[2], faces[i].edgeSmoothness );
			} else if (vf.length > 3) {
		...
		if (tmpSmoothingMethod != Mesh.NO_SMOOTHING) {
			try {
				...
				for (int i = 0; i < vertices.length; i++) {
					...
					// vertex[i].smoothness = ( (Float) vs.elementAt( i )
					// ).floatValue();
				}
				Edge ted;
					...
					for (int i = 0; i < oldParamVal.length; i++) {
						if (oldParamVal[i] instanceof VertexParameterValue) {
							...
							for (int j = 0; j < face.length; ++j)
								newval[j] = oldval[((Integer) faceInfo
							...
							for (int j = 0; j < face.length; ++j) {
								for (int k = 0; k < 3; k++) {
									int pmeFace = ((Integer) faceInfo
											.elementAt(j)).intValue();
									int[] fv = getFaceVertices(faces[pmeFace]);
									int[] vf = ((VertexParamInfo) vertInfo
											.elementAt(triFaces[j][k])).vert;
									double[] coef = ((VertexParamInfo) vertInfo
											.elementAt(triFaces[j][k])).coef;
							...
							newParamVal[i] = new FaceVertexParameterValue(
									newval);
						} else
	...
	public TriangleMesh convertToTriangleMesh(double tol) {
		...
		/*
		 * if ( smoothingMethod != Mesh.NO_SMOOTHING ) { //s1 = new Vector();
		 * //s2 = new Vector(); //s3 = new Vector(); }
		 */
		...
		if (smoothingMethod != Mesh.NO_SMOOTHING) {
			 ...
			 * edge[face[i].e1].smoothness = ( (Float) s1.elementAt( i )
			 * ).floatValue(); edge[face[i].e2].smoothness = ( (Float)
			 * s2.elementAt( i ) ).floatValue(); edge[face[i].e3].smoothness = (
			 * (Float) s3.elementAt( i ) ).floatValue(); }
			 */
	 ...
	 * algorithm. Creasing according to Pixar/Blender algorithm
	 * 
	 */
			...
			for (int i = 0; i < originalVert; ++i) {
				...
				for (int j = 0; j < ve.length; ++j) {
					...
					if (face1 != -1 && face2 != -1)
						...
						smoothness = 6.0; // boundary edges are treated as
					// hard edges.
					if (ns + 1 <= smoothness) {
					...
					case 2:
						...
						sharpPt
								.add(vertices[edges[edges[sharpEdge[0]].next].vertex].r
										.times(0.125));
						sharpPt
								.add(vertices[edges[edges[sharpEdge[1]].next].vertex].r
										.times(0.125));
						if (hardnum == 0)
							newVert[i].r = sharpPt;
						else {
	...
	public void smooth(boolean[] selected, boolean subdivideOnly) {
		...
		if (!subdivideOnly)
			for (int i = originalVert; i < addedVert; ++i) {
				...
				pt.scale(0.25);
				pt2.scale(0.5);
				newVert[i].r = pt; // pt.times( e.smoothness ).plus(
				// pt2.times(1 - e.smoothness ) );
			}
		// location of old vertices
		int n;
		Vec3 pos;
	...
	public void mirrorMesh() {
		...
		if (!changed) {
			...
			for (int i = 0; i < vertices.length; ++i) {
				...
				// newVertices[i + vertices.length].edge =
				// newEdges[newVertices[i].edge].hedge;
			...
			/*
			 * dumpMesh(); Wedge[] te = edges; Wvertex[] tv = vertices; Wface[]
			 * tf = faces; edges = newEdges; vertices = newVertices; faces =
			 * newFaces; dumpMesh(); edges = te; vertices = tv; faces = tf;
			 */
			...
			for (int j = 0; j < boundaries.size(); ++j) {
				...
				// newEdges[e] = new Wedge( newEdges[e].vertex,
				// newEdges[e].hedge, newEdges[newEdges[newe].hedge].face,
				// newEdges[newEdges[newe].hedge].next );
				newEdges[e].face = newEdges[newEdges[newe].hedge].face;
				newEdges[e].next = newEdges[newEdges[newe].hedge].next;
				// System.out.println( "terminating: " + newe + " (" + e + ")" +
				// "face: " + newEdges[newEdges[newe].hedge].face );
			...
			/*
			 * te = edges; tv = vertices; tf = faces; edges = newEdges; vertices =
			 * newVertices; faces = newFaces; //dumpMesh(); edges = te; vertices =
			 * tv; faces = tf;
			 */
			// System.out.println( "edges, newEdges : " + edges.length + " "
			// + newEdges.length + " " );
			int index = 0;
			for (int i = 0; i < newVertices.length; ++i)
