...
public class PolyMesh extends Object3D implements Mesh, FacetedMesh
{
    ...
    /**
     *  Constructor for the PolyMesh object
     *  The spline mesh given as parameter is converted to a polymesh
     *
    ...
    public PolyMesh( SplineMesh smesh )
    {
        ...
        //System.out.println( uSize + " " + vSize + " " + vertices.length + " " + vp.length );
        for ( int i = 0; i < vertices.length; ++i )
        {
            u = (int) Math.round( vertices[i].r.x );
            v = (int) Math.round( vertices[i].r.y );
    ...
    /**
     *  Constructor for the PolyMesh object
     *  The triangle mesh given as parameter is converted to a polymesh
     *
   ...
   */
    ...
    /**
     *  A utility function for resizing a mesh for which some vertices/edges
     *  have been deleted.
     */
    ...
    /**
     * The constructor takes two arguments.  v[] is an array containing the vertices.
     * faces[][] is an array of arrays containing the indices of the vertices which define each
    ...
    public PolyMesh( Vec3[] v, int[][] f )
    {
		...
		for (int i = 0; i < edges.length; i++)
		{
            if (edges[i] != null && edges[i].face == -1 && edges[i].next == -1)
            {
                ...
                for (int j = 0; j < edges.length; j++)
                {
                    if ( edges[j] != null && edges[edges[j].hedge].vertex == n && edges[edges[j].hedge].face == -1
     ...
     *  approximating, the final surface may not actually touch the sides of
     *  this box. If the smoothing method is set to interpolating, the final
    ...
    public RenderingMesh getRenderingMesh( double tol, boolean interactive, ObjectInfo info )
    {
        ...
        int tmpSmoothingMethod = smoothingMethod;
        if ( smoothingMethod == Mesh.APPROXIMATING &&  !finalSmoothing )
        {
            //long t = System.currentTimeMillis();
            PolyMesh mesh = (PolyMesh) duplicate();
            //System.out.println(System.currentTimeMillis() - t);
            //mesh.smoothingMethod = NO_SMOOTHING;
            if (interactive && interactiveSmoothLevel > 0)
            {
                mesh.smoothingMethod = NO_SMOOTHING;
            ...
            else if ( !interactive && renderingSmoothLevel > 0 )
            {
                for (int i = 0; i < renderingSmoothLevel; ++i)
                     mesh.smoothWholeMesh( i, false, smoothingMethod );
                ...
                tmpSmoothingMethod = Mesh.NO_SMOOTHING;
            /*System.out.println( (mesh.time1 - mesh.time2) + " t2: " + mesh.time2 + " t3: " + mesh.time3 + " t4: " + (mesh.time4 - mesh.time2 - mesh.time3) +
                  " t5: " + (mesh.time5 - mesh.time4) +
                  " t6: " + (mesh.time6 - mesh.time5) +
                  " t7: " + (mesh.time7 - mesh.time6));
          System.out.println(System.currentTimeMillis() - t);*/
            //System.out.println(System.currentTimeMillis() - t);
        }
        ...
        /*if ( tmpSmoothingMethod != Mesh.NO_SMOOTHING )
        {
            vs = new Vector();
        }*/
        closed = true;
        ...
        for ( int i = 0; i < vertices.length; ++i )
        {
            ...
            //if ( vs != null )
            //    vs.add( new Float( vertices[i].smoothness ) );
        }
        for ( int i = 0; i < faces.length; ++i )
        {
            ...
            if ( vf.length == 3 )
            {
                ...
                tableInfo.add( new Integer( i ) );
                //if ( s1 != null )
                //    addSmoothness( getFaceEdges( faces[i] ), s1, s2, s3, vf[0], vf[1], vf[2], faces[i].edgeSmoothness );
            }
            else if ( vf.length > 3 )
            {
        ...
        if ( tmpSmoothingMethod != Mesh.NO_SMOOTHING )
        {
            try
            {
                int[][] triFaces = new int[v1.size()][3];
                for ( int i = 0; i < v1.size(); ++i )
                {
                    triFaces[i][0] = ( (Integer) v1.elementAt( i ) ).intValue();
                    triFaces[i][1] = ( (Integer) v2.elementAt( i ) ).intValue();
                    triFaces[i][2] = ( (Integer) v3.elementAt( i ) ).intValue();
                }
                TriangleMesh triMesh = new TriangleMesh( vertArray, triFaces );
                ObjectInfo triInfo = info.duplicate();
                triInfo.object = triMesh;
                Vertex vertex[] = (Vertex[]) triMesh.getVertices();
                Edge edge[] = triMesh.getEdges();
                Face face[] = triMesh.getFaces();
                ...
                for ( int i = 0; i < vertices.length; i++ )
                {
                    ...
                    // vertex[i].smoothness = ( (Float) vs.elementAt( i ) ).floatValue();
                }
                Edge ted;
                    ...
                    for ( int i = 0; i < oldParamVal.length; i++ )
                    {
                        if ( oldParamVal[i] instanceof VertexParameterValue )
                        {
                            ...
                            for ( int j = 0; j < vert.size(); ++j )
                            {
                                ...
                                double[] coef = ( (VertexParamInfo) vertInfo.elementAt( j ) ).coef;
                        ...
                        else if ( oldParamVal[i] instanceof FaceParameterValue )
                        {
                            ...
                            double newval[] = new double[face.length];
                            for ( int j = 0; j < face.length; ++j )
                                newval[j] = oldval[( (Integer) tableInfo.elementAt( j ) ).intValue()];
                        ...
                        else if (oldParamVal[i] instanceof FaceVertexParameterValue)
                        {
                            ...
                            double newval[][] = new double [face.length][3];
                            for ( int j = 0; j < face.length; ++j )
                            {
                                for (int k = 0; k < 3; k++)
                                {
                                    int pmeFace = ( (Integer) tableInfo.elementAt( j ) ).intValue();
                                    int[] fv = getFaceVertices(faces[pmeFace]);
                                    int[] vf = ( (VertexParamInfo) vertInfo.elementAt( triFaces[j][k] ) ).vert;
                                    double[] coef = ( (VertexParamInfo) vertInfo.elementAt( triFaces[j][k] ) ).coef;
                ...
                triMesh.setSmoothingMethod( tmpSmoothingMethod );
                if (controlledSmoothing)
                {
                    Edge[] triedge = triMesh.getEdges();
                    Face[] triface = triMesh.getFaces();
                    MeshVertex[] trivert = triMesh.getVertices();
                ...
                rend = triMesh.getRenderingMesh( tol, interactive, triInfo );
            }
            catch ( Exception e )
            {
                //e.printStackTrace();
                ...
                {
                    triangleMesh.setSmoothingMethod(NO_SMOOTHING);
                }
            }
        }
        if ( rend == null )
        {
            ...
            for ( int i = 0; i < v1.size(); ++i )
                tri[i] = texMapping.mapTriangle( ( (Integer) v1.elementAt( i ) ).intValue(),
                        ( (Integer) v2.elementAt( i ) ).intValue(),
                        ( (Integer) v3.elementAt( i ) ).intValue(), 0, 0, 0, vertArray );
            ...
            if ( oldParamVal != null )
            {
                ...
                for ( int i = 0; i < oldParamVal.length; i++ )
                {
                    ...
                    {
                        double oldval[] = ( (FaceParameterValue) oldParamVal[i] ).getValue();
                        double newval[] = new double[tableInfo.size()];
                    ...
                    else if (oldParamVal[i] instanceof FaceVertexParameterValue)
                    {
                        ...
                        for ( int j = 0; j < v1.size(); ++j )
                        {
                            for (int k = 0; k < 3; k++)
                            {
                                ...
                                int pmeFace = ( (Integer) tableInfo.elementAt( j ) ).intValue();
                                int[] fv = getFaceVertices(faces[pmeFace]);
                                int[] vf = ( (VertexParamInfo) vertInfo.elementAt( vertex ) ).vert;
                                double[] coef = ( (VertexParamInfo) vertInfo.elementAt( vertex ) ).coef;
                                for ( int l = 0; l < vf.length; ++l )
                                {
                                    int vv = -1;
                                    for (int m = 0; m < fv.length; ++m)
     ...
     */
    public PolyMesh getSubdividedMesh()
    {
        if (subdividedMesh == null  && smoothingMethod == Mesh.APPROXIMATING)
        {
            PolyMesh mesh = (PolyMesh) duplicate();
            if (interactiveSmoothLevel > 0)
            {
                mesh.smoothingMethod = NO_SMOOTHING;
                for (int i = 0; i < interactiveSmoothLevel; ++i)
                    mesh.smoothWholeMesh( i, true, smoothingMethod );
    ...
    public RenderingMesh getRenderingMeshQuadCase()
    {
        ...
        for ( int i = 0; i < index; ++i )
            tri[i] = texMapping.mapTriangle( v1vec[i],
                    v2vec[i], v3vec[i], 0, 0, 0, vertArray );
        rend = new RenderingMesh( vertArray, new Vec3[]{null}, tri, texMapping, getMaterialMapping() );
        ParameterValue oldParamVal[] = getParameterValues();
        if ( oldParamVal != null )
        {
     ...
     *  Ear clipping algorithm is used
     *
        ...
        for ( int i = 0; i < selected.length; i++ )
        {
            if ( selected[i] )
            {
                ...
                for ( int j = vertices.length; j < vert.size(); ++j )
                {
                    ...
                    //newVertices[j].smoothness = faces[i].centerSmoothness;
            ...
            for ( int k = 0; k < oldParamVal.length; k++ )
            {
                if ( oldParamVal[k] instanceof FaceParameterValue )
                {
                    ...
                    for ( int j = oldval.length; j < newval.length; j++ )
                        newval[j] = oldval[( (Integer) faceTable.elementAt( j - oldval.length ) ).intValue()];
                ...
                else if ( oldParamVal[k] instanceof VertexParameterValue )
                {
                    ...
                    for ( int j = 0; j < oldval.length; j++ )
                        newval[j] = oldval[j];
                    for ( int j = oldval.length; j < newval.length; j++ )
                        newval[j] = oldval[( (Integer) vertTable.elementAt( j - oldval.length ) ).intValue()];
                    ...
                    for ( int j = 0; j < faces.length; ++j )
                    {
                        ...
                        else
                        {
                            if (j < oldFaceCount)
                                ...
                                faceRef = ((Integer)faceTable.elementAt(j-oldFaceCount)).intValue();
                            ...
                            for (int m = 0; m < 2; m++)
                            {
                                ...
                                else for (int l = 0; l < oldIndices.length; l++)
                                {
    ...
    /**
     * Records the vertices for each selected face
     * or every face is the selection is set to null
     ...
     *  yields the triangulated mesh for the face (ear cutting algorithm).
     *  This method is also used to compute face areas.
     *
    ...
    private double triangulate( int[] vf, int face, boolean areaOnly )
    {
        ...
        while ( count < getDelLength(deleted) && getDelLength(deleted) > 3 )
        {
            ...
            else
            {
                ...
                if ( tableInfo != null )
                    tableInfo.add( new Integer( face ) );
     ...
     */
    private void addSmoothness( int[] fe, Vector s1, Vector s2, Vector s3, int i1, int i2, int i3, float s )
    {
        if ( s1 != null )
        {
        ...
        for ( int i = 0; i < vertices.length; i++ )
        {
            ...
            if ( ve.length > 1 )
            {
                ...
                for ( int j = 0; j < ve.length; ++j )
                {
                    ...
                    //v3 = vertices[i].r.cross( vertices[edges[ve[j]].vertex].r );
                    //v3.add( vertices[edges[ve[j]].vertex].r.cross( vertices[edges[ve[pred]].vertex].r ) );
                    //v3.add( vertices[edges[ve[pred]].vertex].r.cross( vertices[i].r ) );
                    //area = v3.length();
    ...
    /**
     * Sets the mesh vertices, edges and faces array. Use this method after you've changed
     * any mesh feature.
     * @param v
     * @param e
     * @param f
    ...
    /**
     *  Sets the size the PolyMesh object
     *
    ...
    public TriangleMesh convertToTriangleMesh( double tol )
    {
        ...
        for ( int i = 0; i < faces.length; ++i )
        {
            ...
            if ( vf.length == 3 )
            {
                ...
                if (tableInfo != null)
                    tableInfo.add( new Integer(i) );
                //if ( s1 != null )
                //    addSmoothness( getFaceEdges( faces[i] ), s1, s2, s3, vf[0], vf[1], vf[2], 1.0f );
            }
            else if ( vf.length > 3 )
            {
                triangulate( vf, i, false );
        ...
        if ( smoothingMethod != Mesh.NO_SMOOTHING )
        {
           ...
           {
               edge[face[i].e1].smoothness = ( (Float) s1.elementAt( i ) ).floatValue();
               edge[face[i].e2].smoothness = ( (Float) s2.elementAt( i ) ).floatValue();
               edge[face[i].e3].smoothness = ( (Float) s3.elementAt( i ) ).floatValue();
    ...
    {
        //if (triangleMesh == null)
        //    return convertToTriangleMesh(0);
        return triangleMesh;
    }

    /**
     * Call this method to access translation between edges of the original mesh and edges of the trimesh
     *  that is used to represent the polymesh.
     * An index of -1 means that the edge of the trimesh is not an original edge of the polymesh.
     *
     * @return Indices array describing convertion between triangle mesh edges and polymesh edges.
     */
    ...
    {
        PolyMeshEditorWindow ed = new PolyMeshEditorWindow( parent, "PolyMesh '" + info.name + "'",
                info, cb );
    ...
    public boolean[] divideEdges( boolean[] sel, int nseg )
    {
        ...
        for ( int i = 0; i < sel.length; ++i )
        {
            if ( sel[i] )
            {
                ...
                //newVertices[vl + index].smoothness = (float) ( fraction * vertices[to].smoothness + ( 1 - fraction ) * ( vertices[from].smoothness ) );
                toVert[index] = to;
                fromVert[index] = from;
    ...
    public void divideAllEdgesByTwo( )
    {
        ...
        newVertices = new Wvertex[vertices.length + count];
        newEdges = new Wedge[edges.length + 2 * count ];
        int[] newProjectEdges = new int[edges.length + 2 * count];
        ...
        int counter;
        //System.out.println("avant division:" + (t- System.currentTimeMillis()));
        for ( int i = 0; i < count; ++i )
        {
            //System.out.println("pendant division:" + i );
            ...
            //counter = 0;
            //ed = getPreviousEdge(edges[i].hedge);
            //ed = edges[ed].hedge;
            //if (ed >= edges.length/2)
            //        ed += newEdges.length/2 - edges.length/2;
            while ( newEdges[newEdges[ed].hedge].next != newEdges[i].hedge )
            {
                ed = newEdges[newEdges[ed].hedge].next;
                //counter++;
                //if (counter == newEdges.length)
                //    System.out.println( "big pb subdivision");
            ...
            newEdges[el + index + newEdges.length / 2].smoothness = edges[i].smoothness;
            newVertices[to].edge = el + index + newEdges.length / 2;
            //newVertices[vl + index].smoothness = (float) ( 0.5 * vertices[to].smoothness + ( 0.5 ) * ( vertices[from].smoothness ) );
            toVert[index] = to;
            fromVert[index] = from;
            newEdges[newEdges[ed].hedge].next = newEdges[el + index].hedge;
        ...
        //System.out.println("apres division:" + (t- System.currentTimeMillis()));
            ...
            for ( int k = 0; k < oldParamVal.length; k++ )
            {
                if ( oldParamVal[k] instanceof VertexParameterValue )
                {
                    ...
                    for ( int j = 0; j < oldval.length; j++ )
                        newval[j] = oldval[j];
                    for ( int j = oldval.length; j < newVertices.length; ++j )
                        newval[j] = 0.5 * oldval[toVert[j - oldval.length]] + 0.5 * ( newval[fromVert[j - oldval.length]] );
                    newParamVal[k] = new VertexParameterValue( newval );
                }
                else if (oldParamVal[k] instanceof FaceVertexParameterValue)
                {
        ...
        //System.out.println("apres texture division:" + (t- System.currentTimeMillis()));
        edges = newEdges;
        vertices = newVertices;
        resetMesh();
    ...
    public boolean[] mergeEdges( int e1, int e2, boolean center )
    {
        ...
        for ( int j = 0; j < newEdges.length; ++j )
        {
            ...
            //    System.out.println("warning : " + newEdges[j].next + "r�f�renc�");
    ...
    /**
     *  Divides segments according to a fraction array
     *  fractions with value between 0 and 1 indicates which
     *  segments are to be divided and how
     *
    ...
    public boolean[] divideEdges( boolean[] sel, double fraction )
    {
        ...
        for ( int i = 0; i < sel.length; ++i )
        {
            if ( sel[i] )
            {
                ...
                newEdges[el + index + newEdges.length / 2].smoothness = edges[i].smoothness;
                newVertices[to].edge = el + index + newEdges.length / 2;
                //newVertices[vl + index].smoothness = (float) ( val * vertices[to].smoothness + ( 1 - val ) * ( vertices[from].smoothness ) );
                if ( val != fraction )
                {
                    toVert[index] = from;
                    fromVert[index] = to;
    ...
    public void connectVertices( int[] connect )
    {
        ...
        for ( int i = 1; i < connect.length; ++i )
        {
            for ( int j = 0; j < i; ++j )
            {
                ...
                if ( !add )
                {
                    //System.out.println( i1 + " " + i2 + " rejected face share" );
                    continue;
                }
                //System.out.println( i1 + " " + i2 + " candidates for face: " + face );
  ...
  private boolean[] findSingleEdgeLoop(int startEdge)
  {
    ...
    while (true)
    {
      ...
      if ( v.size() > 1)
      {
          ...
          {
              normDir.add( ((Vec3)v.elementAt(i)).cross( (Vec3)v.elementAt(i+1)));
          }
          normDir.normalize();
      }
...
/** Select an edge strip from a single edge.
  ...
  public boolean[] findEdgeStrips(boolean selection[])
  {
     ...
     */
    public PolyMesh( DataInputStream in, Scene theScene )
    ...
    public PolyMesh( DataInputStream in )
        throws IOException, InvalidObjectException
    {
        super();
        initialize();
        ...
        throws IOException, InvalidObjectException
    {
        ...
        for ( int i = 0; i < vertices.length; i++ )
        {
            //System.out.println( "reading vertex : " + i);
        ...
        //System.out.println( edges.length + " edges");
        for ( int i = 0; i < edges.length; i++ )
        {
            //System.out.println( "reading edge : " + i);
            edges[i] = new Wedge( in.readInt(), in.readInt(), in.readInt(), in.readInt() );
            edges[i].smoothness = in.readFloat();
        }
        faces = new Wface[in.readInt()];
        //System.out.println( faces.length + " faces");
        for ( int i = 0; i < faces.length; i++ )
        {
            //System.out.println( "reading face : " + i);
        ...
        //interactiveSmoothLevel = 0;
        //renderingSmoothLevel = 2;
    }

    /**
     *  Smoothes the whole mesh according to Catmull-Clark or Biermann-Levin-Zorin algorithm.
     *  Creasing according to Pixar/Blender algorithm
     *
     */
    public void smoothWholeMesh(int ns, boolean calcProjectedEdges, int smoothingMethod)
    {
        //long t = System.currentTimeMillis();
        //dumpMesh();
        ...
        {
            if (projectedEdges == null || projectedEdges.length != edges.length)
            {
        ...
        divideAllEdgesByTwo();
        //System.out.println("after edge division : " + (System.currentTimeMillis() - t));
        ...
        count /= 2;
        //System.out.println("after face position : " + (System.currentTimeMillis() - t));
        //BTW, now we know how many more edges and faces
        Wedge[] newEdges = new Wedge[edges.length + count * 2];
        if (calcProjectedEdges)
        {
            newProjectedEdges = new int[edges.length + count * 2];
            for (int i = 0; i < edges.length/2; i++)
            {
        ...
        //System.out.println("after calc projected edges : " + (System.currentTimeMillis() - t));
        ...
        int v2;
        int sharp;
        double weight, centralWeight, adjacentWeight;
        int crease;
        int maxHardIndex;
        double maxHard;
        int[] hardEdge = new int[2];
        int[] sharpEdge = new int[2];
        int[] smoothEdge = new int[10];
        ...
        if (smoothingMethod == Mesh.APPROXIMATING)
        {
            ...
            for ( int i = 0; i < originalVert; ++i )
            {
                //adjacent polygons
                int ve[] = getVertexEdges( vertices[i] );
                if (ve.length > smoothEdge.length)
                {
                ...
                for ( int j = 0; j < ve.length; ++j )
                {
                    ...
                    if (face1 != -1 && face2 != -1)
                        smoothness = (1.0 - smoothness ) * 6;
                    else
                        smoothness = 6.0; //boundary edges are treated as hard edges.
                    if ( ns + 1 <= smoothness )
                    {
                    ...
                    switch(sharp)
                    {
                        ...
                        case 1:
                            ...
                            else if (hardnum == 1)
                            {
                                ...
                                sharpPt.add( vertices[edges[edges[maxHardIndex].next].vertex].r.times(0.125) );
            ...
            //System.out.println("after old vertices computation : " + (System.currentTimeMillis() - t));
        ...
        else
        {
            for ( int i = 0; i < originalVert; ++i )
            {
                ...
                {
                    sharpPt = new Vec3( newVert[i].r.times(2*(n-1)) );
                    sharpPt.add( vertices[edges[sharpEdge[0]].vertex].r );
                    sharpPt.add( vertices[edges[sharpEdge[1]].vertex].r );
                    sharpPt.scale( 1.0 / (2*n));
                    if (weight <= 2 )
                    {
                        weight /= 2;
                        newVert[i].r = pos.times( 1 - weight ).plus( sharpPt.times( weight ) );
        ...
        if (smoothingMethod != Mesh.APPROXIMATING)
        {
            for ( int i = originalVert; i < addedVert; ++i )
            {
                ...
                if (mirrorState == NO_MIRROR)
                    ...
                    smoothness = edgeSmoothness[vertices[i].edge];
                smoothness = (1.0 - smoothness ) * 6;
                face1 = e.face;
                ...
                if ( face1 == -1 || face2 == -1 )
                    continue;
                if (ns + 1 <= smoothness || (face1 == -1 || face2 == -1) )
                {
                    //hard edge
        ...
        {
            int v3, v4, v5, v6;
            Vec3 v0r, v1r, v2r, v3r, v4r, v5r, v6r;
            int e1, e2, e1h, e2h;
            double gamma;
            for ( int i = originalVert; i < addedVert; ++i )
            {
                ...
                if (ns + 1 <= smoothness )
                {
                    ...
                    //pt2 = new Vec3(newVert[e.vertex].r);
                    //pt2.add( newVert[edges[edges[e.hedge].next].vertex].r );
                    //pt2 = new Vec3(newVert[edges[e1].vertex].r);
                    //pt2.add( newVert[edges[e2].vertex].r );
                    //pt2.scale( 0.5 );
                    //newVert[i].r = pt2;
                }
                else if ( ns < smoothness )
            ...
            //System.out.println("after new vertices computation : " + (System.currentTimeMillis() - t));
        }
        ...
        for ( int i = 0; i < faceNum; ++i )
        {
            ...
            //newFaces[faceCount].centerSmoothness = faces[i].centerSmoothness;
        ...
        //System.out.println("after faces computation : " + (System.currentTimeMillis() - t));
        ...
        //System.out.println("after texture computation : " + (System.currentTimeMillis() - t));
        vertices = newVert;
        faces = newFaces;
        edges = newEdges;
        resetMesh();
    ...
    /**
       ...
       *
       *@param  subdivideOnly  If true, then no smoothing occurs (subdivision
       *      only)
      ...
      public void smooth( boolean[] selected, boolean subdivideOnly )
      {
              ...
              if ( selected[i] )
              {
                  ...
                  if ( !subdivideOnly )
                  {
                      ...
                      while ( e.next != start )
                      {
                          ...
                          if ( count > edges.length )
                          {
                              System.out.println( "Error in smooth : face is not closed" );
          ...
          for ( int i = 0; i < faces.length; ++i )
          {
              ...
              if ( selectType[i] > 0 )
              {
                  ...
                  //newVert[addedVert + index].smoothness = faces[i].centerSmoothness;
                  vertParamInfo[index] = new VertexParamInfo( vf, coef );
                  index++;
              }
          }
          ...
          if ( !subdivideOnly )
              for ( int i = originalVert; i < addedVert; ++i )
              {
                  ...
                  pt.scale( 0.25 );
                  pt2.scale( 0.5 );
                  newVert[i].r = pt; //pt.times( e.smoothness ).plus( pt2.times(1 - e.smoothness ) );
              }
          //location of old vertices
          int n;
          Vec3 pos;
          ...
          if ( !subdivideOnly )
              for ( int i = 0; i < originalVert; ++i )
              {
                  ...
                  pos.add( pt );
                  pos.add( newVert[i].r.times( ( n * 1.0 - 3.0 ) / ( (double) n ) ) );
                  newVert[i].r = pos; //pos.times( vertices[i].smoothness ).plus( vertices[i].r.times( 1- vertices[i].smoothness) );
          ...
          for ( int i = 0; i < faceNum; ++i )
          {
              ...
              n = vf.length / 2;
              newFaces[faceCount] = new Wface( count );
              //newFaces[faceCount].edgeSmoothness = faces[faceTable[i]].edgeSmoothness;
              //newFaces[faceCount].centerSmoothness = faces[faceTable[i]].centerSmoothness;
        ...
        for ( int i = 0; i < faces.length; ++i )
        {
            if ( selected[i] )
            {
                ...
                for ( int j = 0; j < fv.length; ++j )
                {
                    ...
                    //newVertices[newVertCount + j].smoothness = newVertices[fv[j]].smoothness;
                    ...
                    //newFaces[newFaceCount + j].centerSmoothness = faces[i].centerSmoothness;
    ...
    public void extrudeRegion( boolean[] selected, double value, Vec3 direction, double scale, Vec3 camZ, boolean useNormals, boolean constrainAxis )
    {
                ...
                for ( int j = 0; j < fe.length; ++j )
                {
                    ...
                    if ( selected[k] )
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if ( !vv.contains( (Integer) sclFace[k].elementAt( l ) ) )
        ...
        for ( int i = 0; i < vl; ++i )
        {
            ...
            if ( done[i] )
            {
                ...
                for ( int j = 0; j < ve.length; ++j )
                {
                    if ( isFaceSelected( selected, edges[ve[j]].face ) && !isFaceSelected( selected, edges[edges[ve[j]].hedge].face ) )
                        start = ve[j];
                    if ( isFaceSelected( selected, edges[ve[j]].face ) && ( mean == null ) )
                    {
                        ...
                        for ( int k = 0; k < vv.size(); ++k )
                            mean.add( sclFaceVert[( (Integer) vv.elementAt( k ) ).intValue()] );
                ...
                for ( int j = 0; j < fc.length; ++j )
                {
                    ...
                    newEdges[newEdgeCount + 2 * j + 1].smoothness = 1.0f;
                    newEdges[newEdgeCount + 2 * j + 1 + newEdges.length / 2].smoothness = 1.0f;
                    //System.out.println( "changed next " + nfc[j] + " to " + ( newEdgeCount + 2 * j ) );
                    newEdges[nfc[j]].next = newEdgeCount + 2 * j;
                    newEdges[nfc[j]].face = newFaceCount + j;
                    newFaces[newFaceCount + j] = new Wface( newEdgeCount + 2 * j );
                    //newFaces[newFaceCount + j].edgeSmoothness = faces[currentFace].edgeSmoothness;
                    //newFaces[newFaceCount + j].centerSmoothness = faces[currentFace].centerSmoothness;
                    //newFaces[newFaceCount + j].convex = faces[currentFace].convex;
                    ...
                    if ( changed )
                    {
                        ...
                        //System.out.println( "changed next " + newEdges[prev].hedge + " to " + newNext );
                    }
                }
    ...
    public void facetVertices( boolean[] selected )
    {
        for ( int i = 0; i < selected.length; ++i )
        {
            if ( selected[i] )
            {
                ...
                /*for ( int j = 0; j < ve.length; ++j )
                {
                    //newFaces[faces.length].edgeSmoothness += faces[edges[ve[j]].face].edgeSmoothness;
                    //newFaces[faces.length].centerSmoothness += faces[edges[ve[j]].face].centerSmoothness;
                }*/
                //newFaces[faces.length].edgeSmoothness /= (double) ve.length;
                //newFaces[faces.length].centerSmoothness /= (double) ve.length;
                for ( int j = 0; j < newEdges.length; ++j )
        ...
        for ( int i = 0; i < faces.length; ++i )
        {
            ...
            if ( fe.length == 2 )
            {
                ...
                if ( ve.length == 2 )
                    ...
                    {
                        //lonely vertex
                        selected = deleteDanglingVertex( edges[fe[1]].vertex, selected );
                        --i;
                        continue;
                    }
                //remove the face
     ...
     *  edges. This situation can occur when a two edge face is along a
     *  boundary.
     *
    ...
    public boolean[] mergeEdges( boolean[] selected )
    {
        ...
        for ( int i = 0; i < edges.length; ++i )
        {
            ...
            if ( selected[esel] )
            {
                ...
                if ( esel >= edges.length / 2 )
                    esel = edges[e].hedge;
                //System.out.println( i + " / " + e + " " + edges[e].face + " " + selected[esel] );
            ...
            for ( int l = 0; l < i; ++l )
            {
                ...
                if ( selected[i] && selected[l] )
                {
                    ...
                    if ( count > 1 )
                    {
                        ...
                        new BStandardDialog( PMTranslate.text( "errorTitle" ), UIUtilities.breakString( PMTranslate.text( "illegalMeshStructure" ) ), BStandardDialog.ERROR ).showMessageDialog( null );
                        return selected;
                    }
                    Wedge[] newEdges = new Wedge[edges.length - 2];
                    int[] edgeTable = new int[edges.length];
    ...
    public boolean[] bevelEdges( boolean[] selected, double value )
    {
        ...
        for ( int i = 0; i < vertices.length; ++i )
        {
            ...
            for ( int j = 0; j < e.length; ++j )
            {
                ...
                if ( fullSel[ed] && fullSel[ned] )
                {
                    ...
                    if ( ( prod > 1e-4 ) && ( e.length > 2 ) )
                    {
                        ...
                        n3 = vertices[edges[ned].vertex].r.minus( vertices[i].r ).cross(
                                vertices[edges[edges[ed].hedge].vertex].r.minus( vertices[i].r ) );
                        if ( n3.length() > 1e-6 )
                            n3.normalize();
                        ...
                        //System.out.println( "dot product: " + Math.abs( n1.dot( n2.cross( n3 ) ) ) );
                ...
                else if ( fullSel[ned] )
                {
                    ...
                    if ( j != 0 )
                    {
                        if ( cutsQuality[ped] == ONE_BEVEL && e.length > 2 )
                        {
                            ...
                            --vertAdded;
                            //System.out.println( "vertex MERGED ONE_BEVEL_NEXT" );
                        }
                    }
                }
                else if ( !fullSel[ped] )
            ...
            for ( int j = 0; j < e.length; ++j )
            {
                ...
                if ( ( cutsTable[ed] != -1 ) && ( cuts[ed] == null ) )
                {
                    ...
                    {
                        //intersection of vertex bevel with ed.
                    	//System.out.println( "intersection of vertex bevel with ed" );
                        n3 = normals[i];
                        d = -p3.dot( n3 );
        ...
        /*for ( int i = 0; i < edges.length; ++i )
        {
            System.out.println( "cuts table " + i + " : " + cutsTable[i] + " / " + cutsTable[i + edges.length] );
        ...
        faceCount = faces.length;
        edgeCount = edges.length / 2;
        //System.out.println( edges.length + " " + newEdges.length + " " + edgeCount );
        translateMesh( newVertices, newEdges, newFaces );
        Wedge[] te = edges;
        Wvertex[] tv = vertices;
        Wface[] tf = faces;
        ...
        for ( int i = 0; i < vl; ++i )
        {
            ...
            if ( !cut )
                continue;
            //System.out.println( "Face creation for: " + i + " " + e.length );
            edges = newEdges;
            vertices = newVertices;
            faces = newFaces;
            ...
            if ( count > 2 && e.length < 3 )
            {
                ...
                else if ( newVertices[cutsTable[ed]] == null )
                {
                    ...
                    //newVertices[cutsTable[ed]].smoothness = vertices[i].smoothness;
                }
                ...
                //newVertices[cutsTable[ed + el]].smoothness = vertices[i].smoothness;
                ...
                else if ( newVertices[cutsTable[te[ned].hedge]] == null )
                {
                    ...
                    //newVertices[cutsTable[te[ned].hedge]].smoothness = vertices[i].smoothness;
                }
                ...
                for ( int j = 0; j < e.length; ++j )
                {
                    ...
                    if ( cutsTable[e[next]] == -1 )
                    {
                        ...
                        else if ( newVertices[cutsTable[e[j]]] == null )
                        {
                            ...
                            //newVertices[cutsTable[e[j]]].smoothness = vertices[i].smoothness;
                        }
                    ...
                    if ( first )
                    {
                        ...
                        //System.out.println( ( edgeCount + newEdges.length / 2 ) + " :next: " + ( edgeCount + count - 1 + newEdges.length / 2 ) );
                        first = false;
                    }
                    else
                    ...
                    ++edgeCount;
                    //System.out.println( j + " / " + e.length + " ec: " + edgeCount );
                }
        ...
        vertices = newVertices;
        faces = newFaces;
        //System.out.println( "Face terminée: " + edgeCount + " / " + newEdges.length / 2 );
        //dumpMesh();
        edges = te;
        vertices = tv;
        faces = tf;
        ...
        for ( int i = 0; i < edges.length / 2; ++i )
        {
            ...
            if ( selected[i] )
            {
                ...
                else if ( newVertices[cutsTable[i]] == null )
                {
                    ...
                    //newVertices[cutsTable[i]].smoothness = vertices[edges[i].vertex].smoothness;
                }
                ...
                else if ( newVertices[cutsTable[hed]] == null )
                {
                    ...
                    //newVertices[cutsTable[hed]].smoothness = vertices[edges[edges[i].hedge].vertex].smoothness;
                }
                ...
                else if ( newVertices[cutsTable[ed1]] == null )
                {
                    ...
                    //newVertices[cutsTable[ed1]].smoothness = vertices[edges[i].vertex].smoothness;
                }
                ...
                else if ( newVertices[cutsTable[ed2]] == null )
                {
                    ...
                    //newVertices[cutsTable[ed2]].smoothness = vertices[edges[edges[i].hedge].vertex].smoothness;
                }
                ...
                if ( ref2 == -1 )
                {
                    newEdges[ec] = new Wedge( cutsTable[hed], hec, faceCount, ec1 );
                    newEdges[ec].smoothness = newEdges[i].smoothness;
                    newSel[ec] = true;
                    newEdges[hec] = new Wedge( cutsTable[ed2], ec, newEdges[newEdges[ed].hedge].face, newEdges[newEdges[ed].hedge].next );
                ...
                newEdges[hec1] = new Wedge( cutsTable[hed], ec1, newEdges[ed].face, ed );
    ...
    private void removeSmallEdgeLengths()
    {
        ...
        double elimit = Math.sqrt( edgeLengthLimit );

        for ( int i = 0; i < faces.length; ++i )
        {
            vf = getFaceVertices( faces[i] );
            area = triangulate( vf, 0, true );
            System.out.println( area );
            if ( area < edgeLengthLimit )
            {
                int[] fe = getFaceEdges( faces[i] );
                for (int j = 0; j < fe.length; j++)
                {
                    Vec3 v = vertices[edges[fe[j]].vertex].r.minus( vertices[edges[edges[fe[j]].hedge].vertex].r );
                    if ( v.length() < elimit )
                    {

                        sel[fe[j]] = sel[edges[fe[j]].hedge] = true;
                        remove = true;
                    }
                }
            }
        ...
        for ( int i = 0; i < selected.length; ++i )
            if ( selected[i] )
            {
                ...
                if ( v.length == 2 )
                {
                    ...
                    //vertices[vertices.length - 1].smoothness = vertices[i].smoothness;
                ...
                else
                {
                    ...
                    for ( int j = 1; j < v.length; ++j )
                    {
                        ...
                        //vertices[vl + j - 1].smoothness = vertices[i].smoothness;
                        newSel[vl + j - 1] = true;
                        vertTable[j - 1] = i;
                    }
                    for ( int j = 0; j < v.length; ++j )
    ...
    public PolyMesh getMirroredMesh()
    {
        if (mirroredMesh != null)
        {
            //System.out.println( "non null mirrored mesh : " + invMirroredVerts );
            return mirroredMesh;
        }
    ...
    public void mirrorMesh()
    {
        ...
        else
        {
            ...
            for ( int i = 0; i < vertices.length; ++i )
            {
                ...
                //newVertices[i + vertices.length].edge = newEdges[newVertices[i].edge].hedge;
            ...
            for ( int i = 0; i < edges.length; ++i )
            {
                if ( ( edges[i].face == -1 ) && ( !done[i] ) )
                {
                    ...
                    boundary.add( new Integer( i ) );
                    //System.out.println( "boundary add " + i );
                    if  ( ( mirrorState & MIRROR_ON_XY ) != 0 )
                    {
                    ...
                    while ( current != i )
                    {
                        ...
                        boundary.add( new Integer( current ) );
                        //System.out.println( "boundary add " + current );
                        done[current] = true;
                        if  ( ( mirrorState & MIRROR_ON_XY ) != 0 )
                        {
                            if ( Math.abs( r.z ) > 1e-6 )
                    ...
                    if ( !onMirrorPlane )
                        continue;
                    //boundary on mirror face. Reflection.
                    boundaries.addAll( boundary );
                    //delItems += boundary.size();
                }
            }
            //System.out.println( "c" );
            for ( int j = 0; j < boundaries.size(); ++j )
            {
                ...
                //newEdges[e] = new Wedge( newEdges[e].vertex, newEdges[e].hedge, newEdges[newEdges[newe].hedge].face, newEdges[newEdges[newe].hedge].next );
                newEdges[e].face = newEdges[newEdges[newe].hedge].face;
                newEdges[e].next = newEdges[newEdges[newe].hedge].next;
                //System.out.println( "terminating: " + newe + " (" + e + ")" + "face: " + newEdges[newEdges[newe].hedge].face );
          ...
          vertices = tv;
          faces = tf;*/
            //System.out.println( "edges, newEdges : " + edges.length + " " + newEdges.length + " " );
    ...
    /**
     * This method finds similar faces based on orientation or shape similarities.
     ...
     */ 
    public boolean[] findSimilarFaces( boolean[] selected, boolean normal, double normalTol, boolean loose, double looseTol, boolean strict, double strictTol)
    {
    ...
    public boolean[] findSimilarEdges( boolean[] selected, double tol)
    {
        ...
        for (int i = 0; i < selected.length; i++)
        {
            if (selected[i])
            {
                ...
                for (int j = 0; j < selected.length; j++)
                {
                    diff = length - vertices[edges[j].vertex].r.distance(vertices[edges[edges[j].hedge].vertex].r);
    ...
    public void openSeams()
    {
        ...
        int e, v1, v2, nv1, nv2, bv1, bv2, he, nhe, phe, nbv2;
        ...
        {
        	//System.out.println("translating Mesh " + edgeCount*2 + " " + newEdges.length);
        	edges = newEdges;
...
//      Update the texture parameters.
    ...
    /**
     * Returns an array to a continuous selection of edges.
     * If the selection consists in several interconnected lines
     * then an arbitray line is returned.
     *
     * @param sel The current edge selection
     * @param start An edge along the selected line
    ...
    private int[] findEdgeLine(boolean[] sel, int start)
    {
        ...
        while ( edge != -1 )
        {
            ...
            for (int i = 0; i < ve.length; i++)
            {
                if (ve[i] != edge && ve[i] != edges[edge].hedge && isEdgeSelected(ve[i], seams))
            ...
            for (int i = 0; i < ve.length; i++)
            {
                 if ( edges[ve[i]].face < 0 )
                 {
                     ...
                     if (ve[i] < edges.length/2 )
                        ...
                        v2.add(new Integer(ve[i] + newEdges.length/2 - edges.length/2));
                     fromPoint = edges[ve[i]].vertex;
                     break;
                 }
            }
    ...
    public void applyPoseKeyframe( Keyframe k )
    {
        ...
        /*for ( int i = 0; i < faces.length; i++ )
        {
            //1.0f = (float) key.faceEdgeSmoothness[i];
            //faces[i].centerSmoothness = (float) key.facePointSmoothness[i];
            //faces[i].convex = (boolean) key.faceConvex[i];
        }*/
     ...
     *  topology would cause all keyframes to become invalid. Return an actor
     *  for this mesh.
     *
    ...
    public static class PolyMeshKeyframe extends MeshGesture
    {
         ...
         *  this one and one, two, or three others. These methods should never
         *  be called, since PolyMeshes can only be keyframed by converting them
         *  to Actors.
         *
