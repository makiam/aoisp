...
public class MeshUnfolder {
	...
	protected TriangleMesh trimesh; //the trimesh version of the mesh to unfold
...
//			System.out.println("solution time :" + (solutiontime / 1000.0));
		...
		textArea.append("Total unfolding time: " + Math.round(totaltime / 1000.0) + "s\n\n");
		// now let's build the unfolded mesh
		...
		while (!done) {
			...
			if (index == -1) {
				...
				totaltime = new Date().getTime() - totaltime;
				unfoldedMeshesList.add(computeUnfoldedMesh(vertList, edgeList,
						faceList, uverts, uedges, ufaces));
				textArea.append("     ...done in " + Math.round(totaltime/1000) + "s\n");
				++pieceCount;
			}
		}
		...
		for (int i = 0; i < unfoldedMeshes.length; i++) {
		    ...
		    //unfoldedMeshes[i].setBounds(0.05, 0.95, 0.05, 0.095, true);
		}
		return true;
	}
	 ...
	 * unfolded mesh. Ids are kept, they will make it possible to find back which
	 * face/vertex is concerned during UVMapping edition.
	 * @param textArea 
	 * @param pieceCount 
	 */
	private UnfoldedMesh computeUnfoldedMesh(ArrayList<Integer> vertList,
		...
		FlexCompRowMatrix mat = new FlexCompRowMatrix(2*faces.length, nvars);
		...
		for (int i = 0; i < faces.length; i++) {
			...
			m2yy = m11;
			if (v1 == 0 || v1 == 1) {
				b.add(2*i, -m1xx*vertices[v1].r.x -m1xy*vertices[v1].r.y);
				b.add(2*i+1, -m1yx*vertices[v1].r.x -m1yy*vertices[v1].r.y);
			} else {
			...
			if (v2 == 0 || v2 == 1) {
				b.add(2*i, -m2xx*vertices[v2].r.x -m2xy*vertices[v2].r.y);
				b.add(2*i+1, -m2yx*vertices[v2].r.x -m2yy*vertices[v2].r.y);
			} else {
			...
			if (v3 == 0 || v3 == 1) {
				b.add(2*i, -m3xx*vertices[v3].r.x -m3xy*vertices[v3].r.y);
				b.add(2*i+1, -m3yx*vertices[v3].r.x -m3yy*vertices[v3].r.y);
			} else {
		...
		BiCGstab bicg = new BiCGstab(mtb);
		try {
			bicg.solve(matTmat, mtb, vsol);
		} catch (IterativeSolverNotConvergedException e) {
			//textArea.append("Failure : unfolding did not converge");
			e.printStackTrace();
			//return false;
	...
	private void computeBuildMatrix(double[][] mat, double a1, double a2, double a3) {
		double s = Math.sin(a2)/Math.sin(a3);
		double cs1 = s*Math.cos(a1);
		double sn1 = s*Math.sin(a1);
		mat[0][0] = cs1;
		mat[0][1] = sn1;
		mat[1][0] = -sn1;
		mat[1][1] = cs1;
	...
	/**
	 * Computes J2 matrix as per ABF++ paper
	 * 
	 */
